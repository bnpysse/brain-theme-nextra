---
title: "Paint.NET 实验性支持 WineLinux_全翻译"
tags: ["大咖视点_全文翻译", "Solidot_(奇客科技与极客资讯)"]
---

【文章标题】：Paint.NET 实验性支持 Wine/Linux

【文章正文】：
solidot新版网站常见问题，请点击这里查看。    
消息
				本文已被查看 450 次
			
		
Paint.NET 实验性支持 Wine/Linux
					Windows 上的流行图像编辑软件 Paint.NET 释出了 v5.2 Alpha (build 9739)，实验性的加入了对 Wine/Linux 的支持。想要尝试的 Linux 用户需要：1）使用便捷式版本，安装程序无法工作；WINE 版本至少为 v11.14；添加一个注册表项 wine reg add "HKCU\Software\Wine\DllOverrides" /v d3dcompiler_47 /d native /f，防止 Wine 用自身版本覆盖 DLL；已安装 DXVK； 用 /wine 命令行参数运行。
https://forums.paint.net/topic/134563-%F0%9F%8D%B7-extremely-experimental-winelinux-support-how-to-get-started/
            https://forums.paint.net/topic/134563-%F0%9F%8D%B7-extremely-experimental-winelinux-support-how-to-get-started/

## 🔗 知识库双向关联
- [微软画图和照片应用生成的图像嵌入了看不见的水印_全翻译](/auto-clippings/Auto-简报-微软画图和照片应用生成的图像嵌入了看不见的水印)
- [Windows On ARM 原生应用少你可以试试这些改造方法_简报](/auto-clippings/Auto-简报-Windows-On-ARM-原生应用少你可以试试这些改造方法)
- [Github__AprilNEA_OpenLogi_全翻译](/auto-clippings/Raw-翻译-Github-AprilNEA-OpenLogi)