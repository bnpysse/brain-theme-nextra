---
title: "Photon-Emission-Guided Laser Fault Injection Enables RP2350_全翻译"
tags: ["大咖视点_全文翻译", "Hacker_News_Top_(硅谷极客风向标)"]
---

【文章标题】：Photon-Emission-Guided Laser Fault Injection Enables RP2350 Secure Debug
...
- Rescue reset. Set CTRL.RESCUE_RESTART to1 , then clear it to0 through the RP-AP. The chip resets and remains in boot-ROM wait paths. The signed firmware never runs, sosw_lock[48] is never tightened and stays at the permissive value derived fromPAGE48_LOCK1 —LOCK_S = READ_WRITE .
- Fault DEBUGEN to0xc . With both cores in boot-ROM wait paths, setPROC1 andPROC1_SECURE as described above; these two set bits produce the value0xc .
- Halt core 1 through its Debug Halting Control and Status Register (DHCSR ) over the now-Secure Mem-AP.
- Read the secret from OTP rows 0xc08 –0xc0f through the guarded read interface.
...
Footnotes
- 
https://github.com/raspberrypi/rp2350_hacking_challenge The RP2350 Hacking Challenge repository, containing the reference lockdown configuration and firmware we replicated. ↩
- 
Courk, Laser Fault Injection on a Budget: RP2350 Edition. ↩
- 
The rescue check is step 1 of the core 0 boot path in src/main/arm/varm_boot_path.c ; insrc/main/arm/arm8_bootrom_rt0.S ,varm_wait_rescue enters the interrupt-disabledvarm_dead_quiet WFI loop while core 1 remains in the boot ROM’s wait-for-vector path. ↩